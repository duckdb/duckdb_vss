...
#include "hnsw/hnsw_physical_index_join.hpp"

namespace duckdb {

class HNSWIndexJoinOperatorState : public CachingOperatorState {
public:
	HNSWIndexJoinOperatorState(ClientContext &context, const PhysicalHNSWIndexJoin &op)
...
PhysicalHNSWIndexJoin::PhysicalHNSWIndexJoin(LogicalOperator &op, unique_ptr<PhysicalOperator> left,
    ...
    : CachingPhysicalOperator(PhysicalOperatorType::EXTENSION, std::move(op.types), estimated_cardinality),
...
unique_ptr<OperatorState> PhysicalHNSWIndexJoin::GetOperatorState(ExecutionContext &context) const {
	return make_uniq<HNSWIndexJoinOperatorState>(context.client, *this);
}

void PhysicalHNSWIndexJoin::Output(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
	...
	auto &state = state_p.Cast<HNSWIndexJoinOperatorState>();
...
void PhysicalHNSWIndexJoin::GetRHSMatches(ExecutionContext &context, DataChunk &input, OperatorState &state_p) const {
...
OperatorResultType PhysicalHNSWIndexJoin::ExecuteInternal(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
...
//===--------------------------------------------------------------------===//
void PhysicalHNSWIndexJoin::BuildPipelines(Pipeline &current, MetaPipeline &meta_pipeline) {
...
vector<const_reference<PhysicalOperator>> PhysicalHNSWIndexJoin::GetSources() const {
